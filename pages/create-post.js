...
function CreatePost() {
    ...
    async function savePost(hash) {
        if (typeof window.ethereum !== 'undefined') {
            ...
            const contract = new ethers.Contract(contractAddress, Web3Blog.abi, signer)
    ...
    return (
        <div className={container}>
            ...
            <SimpleMDE className={mdEditor} placeholder='What is on your mind?' value={post.content} onChange={value => setPost({ ...post, content: value })} />
            {
                loaded && (
                    <>
                    <button className={button} type='button' onClick={createNewPost}>Publish</button>
                    <button onClick={triggerOnChange} className={button}>Add cover image</button>
                    </>
                )
            }
